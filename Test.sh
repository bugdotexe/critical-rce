...
rust-name() {
  local pkg=$1
  local code
  code=$(curl -Ls -o /dev/null -w "%{http_code}" "https://crates.io/crates/$pkg")
  if [ "$code" -eq 404 ]; then
    printf '\e[1;33m[WARN]\e[0m %s\n' "$pkg is available"
  fi
}
...
getDependencies() {
  ...
  # Alternative Go module extraction for single-line requires
  find "$OUTPUT" -name "go.mod" | \
  ...
  find "$OUTPUT" -name "Dockerfile" -o -name "*.yaml" -o -name "*.yml" | \
    xargs -I {} grep -h "image:" {} | awk -F ':' '{print $2}' | sed 's/"//g' | sed "s/'//g" | awk '{$1=$1};1' | grep -v "^$" | sort -u | anew "$OUTPUT/DEP/docker.deps"
}

checkDependencies() {
  export -f gem-name go-name rust-name docker-name
  notice "Checking npm..."
